...
- **20 July 2026 scheduled corruption-robustness milestone — completed early on 18 July:** deterministic brightness, contrast, blur, noise, and JPEG evaluation at three severities; validation-only selection of four-view dihedral TTA; and an untouched-test single-view result. TTA was rejected on validation, while the baseline retained 0.991879 mean corruption macro F1 and 0.918319 macro F1 under the worst tested condition.
...
- `configs/evaluation/robustness_v1.json` fixes the synthetic-corruption matrix, deterministic seed, validation-selected TTA candidate, and production claim boundary.
...
- `reports/robustness_evaluation_2026-07-18.json` records the validation-selected TTA decision and single-view clean, mean-corruption, severity, family, and worst-condition results.
...
- `docs/CORRUPTION_ROBUSTNESS.md` documents the 20 July robustness phase, rejected TTA candidate, verified baseline result, and synthetic-evidence limits.
